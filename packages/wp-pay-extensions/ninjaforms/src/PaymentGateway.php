...
final class PaymentGateway extends NF_Abstracts_PaymentGateway {
	...
	public function __construct() {
		$this->_name = __( 'Pronamic Pay', 'pronamic-pay-with-mollie-for-ninja-forms' );
	...
	public function process( $action_settings, $form_id, $data ) {
		...
		if ( empty( $description ) ) {
			$description = \sprintf(
				'%s #%s',
				__( 'Submission', 'pronamic-pay-with-mollie-for-ninja-forms' ),
				$payment->source_id
			);
		}
		...
		$payment->title = sprintf(
			...
			__( 'Payment for %s', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			$description
		);
		...
		try {
			...
			$data['actions']['success_message'] = __( 'Please wait while you are redirected to complete the payment.', 'pronamic-pay-with-mollie-for-ninja-forms' );
	...
	public function action_settings() {
		...
		// Configuration.
		$settings['config_id'] = [
			'label'   => __( 'Configuration', 'pronamic-pay-with-mollie-for-ninja-forms' ),
		...
		foreach ( Plugin::get_config_select_options() as $value => $label ) {
			if ( 0 === $value ) {
				$label = \__( '— Default Gateway —', 'pronamic-pay-with-mollie-for-ninja-forms' );
			}
		...
		$settings['description'] = [
			...
			'type'           => 'textbox',
			'group'          => 'pronamic_pay',
			'label'          => __( 'Transaction Description', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			'placeholder'    => '',
			'value'          => '',
			'width'          => 'full',
			'use_merge_tags' => [
		...
		$settings['pronamic_pay_status_pages'] = [
			...
			'type'     => 'fieldset',
			'label'    => __( 'Payment Status Pages', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			'width'    => 'full',
			'group'    => 'pronamic_pay',
			'settings' => [],
		];
		...
		$options = [
			[
				'label' => __( '— Select —', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			],
		];
		...
		// Add settings fields.
		$pages = [
			'completed' => \__( 'Completed', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			'cancel'    => \__( 'Canceled', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			'expired'   => \__( 'Expired', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			'error'     => \__( 'Error', 'pronamic-pay-with-mollie-for-ninja-forms' ),
			'unknown'   => \__( 'Unknown', 'pronamic-pay-with-mollie-for-ninja-forms' ),
		];
		...
		if ( null !== $form_id ) {
			$settings['pronamic_pay_delayed_actions'] = [
				...
				'type'     => 'fieldset',
				'label'    => __( 'Delayed actions', 'pronamic-pay-with-mollie-for-ninja-forms' ),
				'width'    => 'full',
				'group'    => 'pronamic_pay',
				'settings' => [],
			];
